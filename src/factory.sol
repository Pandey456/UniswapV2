...
contract factory {
    ...
    function createPool(address _tokenA , address _tokenB) external onlyRouter returns(address){
        //checks
        require(_tokenA != _tokenB,"Same Token");
        (address token0, address token1) = _tokenA < _tokenB ? (_tokenA, _tokenB) : (_tokenB, _tokenA);
        require(token0 != address(0),"Zero Address" );
        require(poolRegistry[token0][token1] == address(0), "Pool Already Exists");
